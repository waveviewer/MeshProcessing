...
void GH_QEM_simplification(SM &surface_mesh, const double &ratio) {
  glogger->info("MODIFIED GH_QEM_simplification() start");
  glogger->info("multiple triangle area into Q error metric");
  ...
  UV_pmap uv_pmap = surface_mesh.add_property_map<vertex_descriptor, Kernel::Point_2>("v:uv").first;
...
//  My_visitor vis();
  ...
  glogger->info("MODIFIED GH_QEM_simplification, {} edges collapsed in {} ms", deleted_edges_num, collapse_duration.count());
}
...
void GH_keep_boundary(SM &surface_mesh, const double &ratio){
  ...
  Border_is_constrained_edge_map bem(surface_mesh);
  GH_policies gh_policies(surface_mesh);
  typedef SMS::Constrained_placement<GH_policies, Border_is_constrained_edge_map> Constrained_GH_Placement;
...
//      SMS::edge_collapse(surface_mesh, stop,
//                         CGAL::parameters::get_cost(gh_cost).get_placement(gh_placement));
  typedef SMS::Constrained_placement<GH_placement, Border_is_constrained_edge_map> Constrained_GH_placement;
  int deleted_edges_num = SMS::edge_collapse(surface_mesh, stop,
                                             CGAL::parameters::edge_is_constrained_map(bem)
                                                 .get_placement(Constrained_GH_placement(bem)));
